...
func (r *UserRepository) Search(ctx context.Context, query string, limit, offset int) ([]*entities.User, error) {
	searchQuery := "%" + query + "%"
	models, err := gorm.G[models.User](r.db).Where("email ILIKE ? OR username ILIKE ? OR first_name ILIKE ? OR last_name ILIKE ?", searchQuery, searchQuery, searchQuery, searchQuery).Limit(limit).Offset(offset).Find(ctx)
	if err != nil {
